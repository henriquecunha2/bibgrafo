...
class MeuGrafo(GrafoMatrizAdjacenciaNaoDirecionado):
    ...
    def vertices_nao_adjacentes(self):
        ...
        for i in range(len(self._vertices)):
            for j in range(len(self._vertices)):
                if i != j and j > i and not bool(self._matriz[i][j]):
                    nao_adjacentes.add('{}-{}'.format(self._vertices[i], self._vertices[j]))
    ...
    def arestas_sobre_vertice(self, V):
        ...
        for i in range(len(self._matriz)):
            if bool(self._matriz[v][i]) and v <= i:
                for k in self._matriz[v][i]:
                    arestas.add(k)
            if bool(self._matriz[i][v]) and i <= v:
